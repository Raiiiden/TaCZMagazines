...
public class LoadOneFromHandPacket {
    ...
    public static void handle(LoadOneFromHandPacket msg, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            if (!DefaultAssets.EMPTY_AMMO_ID.equals(magAmmoId) && !magAmmoId.equals(familyAmmo)) return;

            boolean creative = player.getAbilities().instabuild;
            if (!creative && !MagazineAmmoSource.takeOneFromInventory(player, familyAmmo)) return;
            ...
            magItem.setAmmoId(held, familyAmmo);
